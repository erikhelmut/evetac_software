...
- `visualize.py` — live OpenCV window: events + tracked dots with magnified displacement arrows,
  rolling plots of event rate, displacement magnitude and mean shift.
...
Common options: `--calibration path.pkl`, `--rate 50` (Hz), `--crop -N`, `--noise-filter-ms 100`,
`--fast` (replay without real-time pacing).

Visualizer keys: `q`/Esc quit · space pause display · `t` toggle tracking · `r` current dots → reference ·
`s` store calibration · `c` clear plots · `+`/`-` arrow gain · `d` event persistence · `h` help.
...
with EvetacReader(CameraSource(), rate_hz=50) as reader:
    for frame in reader.frames():
        ...
        frame.dots_xy            # (63, 2) tracked dot centers (x, y)
        frame.displacement_xy    # (63, 2) displacement from the reference
        frame.event_image(reader.resolution)   # uint8 image, 127 = no event
```

For non-blocking use (e.g. inside a control loop) call `reader.poll()` repeatedly; it returns a
frame or `None`.
